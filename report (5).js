...
function sec(t,m){
  ...
  if(i===-1)i=t.indexOf(m.replace(':\n',':'));
  if(i===-1)return'—';
  var rest=t.substring(i+m.length);
  var e=rest.search(/\n[A-Z][A-Z_0-9]+[:\s]/);
...
function exportPDF(){
  ...
  var h='';
  slides.forEach(function(s){h+='<div class="pg">'+s.html+'</div>';});
  ...
  var css='*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;box-sizing:border-box}'+
    ...
    '.rh{background:rgba(220,50,50,.1)!important;color:#c03030}'+
    '.rm{background:rgba(200,130,0,.1)!important;color:#c07020}'+
    '.rl{background:rgba(30,160,80,.1)!important;color:#1a7040}'+
    '.recbad{background:rgba(30,160,80,.1)!important;color:#1a7040;border:1px solid rgba(30,160,80,.25);border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700}'+
  ...
  win.document.write('<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>'+title+'</title><style>'+css+'</style></head><body>');
...
function exportWord(){
  ...
  try{
    ...
    var body='<div style="background:#0d5c5c;color:#fff;padding:60px 40px;text-align:center"><div style="font-size:22pt;font-weight:900;color:#c9a227;margin-bottom:14px">\u062A\u0642\u0631\u064A\u0631 \u0627\u0644\u062F\u0631\u0627\u0633\u0627\u062A \u0627\u0644\u062E\u0645\u0633\u0629</div><div style="font-size:15pt;font-weight:900;color:#fff;margin-bottom:10px">'+title+'</div><div style="font-size:10pt;color:rgba(255,255,255,.7)">'+meta+'</div></div>';
    slides.forEach(function(s){
      if(s.t==='cover')return;
      var d=document.createElement('div');d.innerHTML=s.html;
      if(s.t==='sec'){
        var h=d.querySelector('h2');
        if(h)body+='<div style="background:#085f5f;color:#c9a227;padding:18px 28px;margin-top:0;page-break-before:always"><div style="font-size:16pt;font-weight:900">'+h.textContent+'</div></div>';
        return;
      }
      var hdr=d.querySelector('.cs-hdr h3');var bdg=d.querySelector('.cs-bdg');var bd=d.querySelector('.cs-body');
      if(!bd)return;
      ...
      body+='<div style="margin:12px 0;border:1px solid #cde;border-radius:6px;overflow:hidden;page-break-inside:avoid">'+
        '<div style="background:#0d7a7a;color:#fff;padding:9px 16px;display:flex;justify-content:space-between">'+
        '<strong style="font-size:12pt">'+(hdr?hdr.textContent:'')+'</strong>'+
        '<span style="font-size:9pt;opacity:.8">'+(bdg?bdg.textContent:'')+'</span></div>'+
        '<div style="padding:16px 20px;direction:rtl;font-size:10.5pt;line-height:1.9">'+clean+'</div></div>';
    });
    var wstyle='body{font-family:Arial,sans-serif;direction:rtl;margin:1.8cm;font-size:10.5pt;color:#111;line-height:1.9}'+
      'h4{color:#085f5f;border-right:4px solid #c9a227;padding-right:10px;margin:14px 0 6px;font-size:11pt}'+
      'table{width:100%;border-collapse:collapse;margin:10px 0;font-size:10pt}'+
      'th{background:#0d7a7a;color:#fff;padding:8px 12px;text-align:right;font-weight:bold}'+
      ...
      'li{padding:6px 12px;margin:4px 0;background:#f0f8f8;border-right:3px solid #0d7a7a;line-height:1.7}'+
      'p{margin:6px 0;line-height:1.9}'+
      '.hbox{background:#f0f8f8;border-right:4px solid #0d5c5c;padding:10px 14px;margin:8px 0;line-height:1.9;border-radius:4px}'+
    ...
    var page='<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>'+title+'</title><style>'+wstyle+'</style></head><body>'+body+'</body></html>';
    var blob=new Blob(['\ufeff'+page],{type:'text/html;charset=utf-8'});
    var url=URL.createObjectURL(blob);
    var a=document.createElement('a');
    a.href=url;a.download='\u062A\u0642\u0631\u064A\u0631_'+title.replace(/\s+/g,'_').substring(0,25)+'.doc';
