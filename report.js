...
function exportWord(){
  ...
  try{
    ...
    var body='<div style="background:#0d5c5c;color:#fff;padding:50px;text-align:center"><div style="font-size:20px;font-weight:900;color:#c9a227;margin-bottom:10px">تقرير الدراسات الخمسة</div><div style="font-size:16px;font-weight:900;margin-bottom:6px">'+title+'</div><div style="font-size:11px;opacity:.7">'+meta+'</div></div>';
    slides.forEach(function(s){
      if(s.t==='cover')return;
      var d=document.createElement('div');d.innerHTML=s.html;
      if(s.t==='sec'){var h=d.querySelector('h2');if(h)body+='<h2 style="background:#085f5f;color:#c9a227;padding:16px 24px;margin:0;font-size:17px;page-break-before:always">'+h.textContent+'</h2>';return;}
      var hdr=d.querySelector('.cs-hdr h3');var bdg=d.querySelector('.cs-bdg');var bd=d.querySelector('.cs-body');
      if(!bd)return;
      var clean=bd.innerHTML.replace(/class="[^"]*"/g,'').replace(/style="direction:ltr;/g,'style="');
      body+='<div style="margin:10px 0;border:1px solid #cce;border-radius:6px;overflow:hidden"><div style="background:#0d7a7a;color:#fff;padding:9px 14px;display:flex;justify-content:space-between"><b style="font-size:13px">'+(hdr?hdr.textContent:'')+'</b><span style="font-size:10px;opacity:.8">'+(bdg?bdg.textContent:'')+'</span></div><div style="padding:14px;font-family:Arial,sans-serif;direction:rtl;font-size:10.5pt;line-height:1.8">'+clean+'</div></div>';
    });
    var page='<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>'+title+'</title><style>body{font-family:Arial,sans-serif;direction:rtl;margin:1.5cm;font-size:10.5pt;color:#111;line-height:1.8}table{width:100%;border-collapse:collapse;margin:8px 0;font-size:10pt}th{background:#0d7a7a!important;color:#fff!important;padding:7px 10px;text-align:right;-webkit-print-color-adjust:exact;print-color-adjust:exact}td{padding:6px 10px;border:1px solid #ddd;vertical-align:top}tr:nth-child(even) td{background:#f5fafa!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}ul{list-style:none;padding:0;margin:6px 0}li{padding:5px 10px;margin:3px 0;background:#f0f8f8!important;border-right:3px solid #0d7a7a;-webkit-print-color-adjust:exact;print-color-adjust:exact}h4{color:#085f5f;border-right:3px solid #c9a227;padding-right:8px;margin:12px 0 5px}@page{size:A4;margin:1.5cm}</style></head><body>'+body+'</body></html>';
    ...
    win.document.write(page);win.document.close();win.focus();
    btn.textContent='✅ تم — اضغط Ctrl+P للطباعة أو الحفظ PDF';
