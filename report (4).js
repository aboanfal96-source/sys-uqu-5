...
function sec(t,m){
  if(!t)return'—';
  var i=t.indexOf(m);
  if(i===-1)i=t.indexOf(m.replace(':\n',': '));
  if(i===-1)i=t.indexOf(m.replace(':\n',':'));
  if(i===-1)return'—';
  var mlen=m.length,rest=t.substring(i+mlen);
  var e=rest.search(/\n[A-Z][A-Z_0-9]+[:\s]/);
...
function exportPDF(){
  var title=document.getElementById('rTitle').textContent;
  var allHTML='';
  slides.forEach(function(s){allHTML+='<div class="pg">'+s.html+'</div>';});
  var win=window.open('','_blank');
  if(!win){alert('يرجى السماح بالنوافذ المنبثقة في إعدادات المتصفح');return;}
  win.document.write('<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8">'+
    ...
    'html,body{margin:0;padding:0;font-family:Cairo,Arial,sans-serif;direction:rtl}'+
    ':root{--teal:#0d7a7a;--gold:#c9a227;--gl:#e8c94a;--tl:#0d7a7a;--r:12px}'+
    ...
    '.cv-logo-box{background:rgba(255,255,255,.95);border-radius:14px;padding:14px 20px;display:flex;align-items:center;flex-shrink:0}'+
    ...
    '.cv-univ-ar{font-size:20px;font-weight:900;color:#fff;line-height:1.3}'+
    ...
    '.cv-five-badge{display:inline-block;border:1.5px solid rgba(201,162,39,.5);border-radius:6px;padding:8px 30px;margin-bottom:18px}'+
    ...
    '.si h2{font-size:36px;font-weight:900;color:#e8c94a;margin:0}'+
    ...
    '.cs-bdg{background:rgba(201,162,39,.2)!important;border:1px solid rgba(201,162,39,.4);border-radius:5px;padding:3px 10px;font-size:10px;font-weight:700;color:#e8c94a}'+
    ...
    '.icard .ct{font-size:10px;font-weight:700;color:#085f5f;margin-bottom:4px}'+
    '.icard p{font-size:12px;margin:0}'+
    ...
    '.rm{background:rgba(200,130,0,.1)!important;color:#c07020}'+
    '.rl{background:rgba(30,160,80,.1)!important;color:#1a7040}'+
    '.recbad{background:rgba(30,160,80,.1)!important;color:#1a7040;border:1px solid rgba(30,160,80,.25);border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700;display:inline-flex;gap:3px}'+
    ...
    '.tl-box{background:rgba(13,122,122,.04)!important;border:1px solid rgba(13,122,122,.1);border-radius:7px;padding:8px 11px;flex:1}'+
    ...
    '.cs-ftr{background:rgba(13,122,122,.04)!important;border-top:1px solid rgba(13,122,122,.08);padding:6px 26px;display:flex;justify-content:space-between}'+
...
function exportWord(){
  ...
  try{
    var title=document.getElementById('rTitle').textContent;
    var meta=document.getElementById('rMeta').textContent;

    // بناء محتوى الوورد
    var body='';

    // غلاف
    body+='<div style="background:#0d5c5c;color:#fff;padding:60px 40px;text-align:center;page-break-after:always">';
    body+='<div style="font-size:14pt;color:#c9a227;margin-bottom:16px;letter-spacing:2px">جامعة أم القرى · وكالة الجامعة للمشاريع</div>';
    body+='<div style="width:120px;height:3px;background:#c9a227;margin:0 auto 24px"></div>';
    body+='<div style="font-size:22pt;font-weight:900;color:#c9a227;margin-bottom:16px">تقرير الدراسات الخمسة</div>';
    body+='<div style="font-size:16pt;font-weight:900;color:#fff;margin-bottom:12px">'+title+'</div>';
    body+='<div style="font-size:11pt;color:rgba(255,255,255,.7)">'+meta+'</div>';
    body+='</div>';
    ...
    slides.forEach(function(s){
      ...
      if(s.t==='sec'){
        ...
        var desc=d.querySelector('.si-desc');
        if(h){
          body+='<div style="background:#085f5f;color:#c9a227;padding:20px 30px;margin-top:0;page-break-before:always">';
          body+='<div style="font-size:18pt;font-weight:900;margin-bottom:8px">'+h.textContent+'</div>';
          if(desc)body+='<div style="font-size:10pt;color:rgba(255,255,255,.7)">'+desc.textContent+'</div>';
          body+='</div>';
        }
        return;
      }
      var hdr=d.querySelector('.cs-hdr h3');
      var bdg=d.querySelector('.cs-bdg');
      var bd=d.querySelector('.cs-body');
      if(!bd)return;
      // تنظيف الكود
      var clean=bd.innerHTML
        .replace(/class="hbox gld"/g,'class="hbox"')
        .replace(/class="rbad rh"/g,'style="color:#c03030;font-weight:bold;background:#fff0f0;padding:2px 6px;border-radius:3px"')
        .replace(/class="rbad rm"/g,'style="color:#c07020;font-weight:bold;background:#fff8f0;padding:2px 6px;border-radius:3px"')
        .replace(/class="rbad rl"/g,'style="color:#1a7040;font-weight:bold;background:#f0fff4;padding:2px 6px;border-radius:3px"')
        .replace(/class="recbad"/g,'style="color:#1a7040;font-weight:bold"')
        .replace(/class="[^"]*"/g,'')
        .replace(/contenteditable="[^"]*"/g,'')
        .replace(/style="direction:ltr;/g,'style="');
      body+='<div style="margin:16px 0;border:1px solid #cdd;border-radius:6px;overflow:hidden;page-break-inside:avoid">';
      body+='<div style="background:#0d7a7a;color:#fff;padding:10px 16px;display:flex;justify-content:space-between;align-items:center">';
      body+='<strong style="font-size:12pt">'+(hdr?hdr.textContent:'')+'</strong>';
      body+='<span style="font-size:9pt;opacity:.8">'+(bdg?bdg.textContent:'')+'</span>';
      body+='</div>';
      body+='<div style="padding:16px 20px;font-family:Arial,sans-serif;direction:rtl;font-size:10.5pt;line-height:1.9">'+clean+'</div>';
      body+='</div>';
    });

    var fullHTML='<!DOCTYPE html>\n<html dir="rtl" lang="ar">\n<head>\n<meta charset="UTF-8">\n<title>'+title+'</title>\n'+
      '<style>\n'+
      'body{font-family:Arial,sans-serif;direction:rtl;margin:2cm;font-size:10.5pt;color:#111;line-height:1.9}\n'+
      'h4{color:#085f5f;border-right:4px solid #c9a227;padding-right:10px;margin:14px 0 6px;font-size:11pt}\n'+
      'table{width:100%;border-collapse:collapse;margin:10px 0;font-size:10pt}\n'+
      'th{background:#0d7a7a;color:#fff;padding:8px 12px;text-align:right;font-weight:bold;-webkit-print-color-adjust:exact;print-color-adjust:exact}\n'+
      'td{padding:7px 12px;border:1px solid #cdd;vertical-align:top;line-height:1.7}\n'+
      'tr:nth-child(even) td{background:#f5fafa}\n'+
      'ul{list-style:none;padding:0;margin:8px 0}\n'+
      'li{padding:6px 12px;margin:4px 0;background:#f0f8f8;border-right:3px solid #0d7a7a;line-height:1.7;-webkit-print-color-adjust:exact;print-color-adjust:exact}\n'+
      'li::before{content:"◈ ";color:#0d7a7a}\n'+
      'p{margin:6px 0;line-height:1.9}\n'+
      '.hbox{background:#f0f8f8;border-right:4px solid #0d5c5c;padding:10px 14px;margin:8px 0;line-height:1.9;border-radius:4px;-webkit-print-color-adjust:exact;print-color-adjust:exact}\n'+
      '.tcols{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:10px 0}\n'+
      '.icard{border:1px solid #cdd;padding:10px 12px;border-radius:4px}\n'+
      '.ct{font-size:9pt;font-weight:bold;color:#0d5c5c;margin-bottom:4px}\n'+
      '.tl-row{display:flex;gap:10px;margin-bottom:8px}\n'+
      '.tl-mark{width:20px;flex-shrink:0;text-align:center;color:#0d7a7a;font-weight:bold}\n'+
      '.tl-box{flex:1;border:1px solid #cdd;padding:8px 12px;border-radius:4px}\n'+
      '.pt{font-weight:bold;color:#0d5c5c}\n'+
      '.pm,.pd{font-size:9.5pt;color:#666;margin-top:3px}\n'+
    ...
    a.href=url;
    a.download='تقرير_الدراسات_الخمسة_'+title.replace(/\s+/g,'_').substring(0,30)+'.doc';
